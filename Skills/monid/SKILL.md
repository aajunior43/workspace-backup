...
For the most up-to-date command signatures, run `monid --help` and `monid  --help`.
---
## Setup
...
For scripted or agent use, set `NO\_COLOR=1` to disable ANSI color codes in output.
---
## When to Use Monid
...
1. \*\*Discover\*\* — Run `monid discover -q ""` to search available tools. Use `-s ` to filter by minimum relevance. Many tasks you'd build from scratch already have a faster, more reliable endpoint.
...
|---------|-------------|
| `monid discover` | Search for data endpoints using natural language (`-q `, `-l `, `-s `) |
| `monid inspect` | Get full details and input schema for a specific endpoint (`-p  -e `) |
...
| `monid runs get` | Get run status and results (`-r `, `-w` to wait) |
| `monid balance` | Show current workspace balance |
| `monid keys add` | Add an API key (`-k  -l `) |
| `monid keys list` | Show configured keys |
| `monid keys remove` | Remove a key (`-l `, `-f` to skip confirmation) |
| `monid keys activate` | Switch the active key (`-l `) |
...
monid runs get -r 01HXYZ... -o ai\_tweets.json
# -> status: COMPLETED
```
...
monid runs get -r 01HTWIT... -o twitter\_ai.json
monid runs get -r 01HLINK... -o linkedin\_ai.json
...
## Key Management
```bash
monid keys add -k  -l main # Add a key (first key is auto-activated)
monid keys list # Show all configured keys
monid keys activate -l main # Switch the active key
monid keys remove -l main # Remove a key (use -f to skip confirmation)
```
API key format: `monid\_\_` (e.g. `monid\_live\_abc123...`). Generate keys at https://app.monid.ai/access/api-keys.
---
## Run Statuses
| Status | Meaning |
|--------|---------|
...
- Poll with `monid runs get -r ` every 5-10 seconds
...
\*\*Saving output:\*\*
- Always use `-o ` to save results to a file once the run completes (works with both approaches)
---
## Troubleshooting
\*\*"No active API key"\*\* — No key configured. Run `monid keys add -k  -l main`.
...
\*\*Run status FAILED\*\* — Check error details with `monid runs get -r `. Common causes: invalid input parameters (re-inspect the endpoint), rate limits (retry later), or request scope too large (reduce item count).
\*\*Run taking a long time\*\* — Normal for some endpoints. Runs can take up to 120 seconds. Keep polling or let `--wait` handle it.
---
## Rules for Agents
1. \*\*Discover first\*\* — before writing a scraper, calling a third-party API directly, or telling the user you can't access something — run `monid discover` to see if a better tool exists. The catalog grows continuously and you don't know what's available until you search.
...
5. \*\*Always use `-o `\*\* to save results to a file once the run completes.
